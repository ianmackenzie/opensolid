...
startPoint (Curve curve _ _) = startPointImpl curve

endPoint :: Curve2d (space @ units) -> Point2d (space @ units)
endPoint (Line _ p2 _) = p2
endPoint arc@(Arc{}) = evaluateAt 1.0 arc
endPoint (Curve curve _ _) = endPointImpl curve
...
evaluateAt t (Curve curve _ _) = Curve2d.Internal.evaluateAtImpl t curve
...
segmentBounds t (Curve curve _ _) =
...
derivative (Curve _ first _) = first
...
reverse (Curve curve first second) =
  Curve
