...
instance (units ~ units', coordinates ~ coordinates') => Subtraction (Point3d units coordinates) (BoundingBox3d units' coordinates') (VectorBox3d units coordinates) where
  Point3d px py pz - BoundingBox3d bx by bz = VectorBox3d (px - bx) (py - by) (pz - bz)

instance (units ~ units', coordinates ~ coordinates') => Subtraction (BoundingBox3d units coordinates) (Point3d units' coordinates') (VectorBox3d units coordinates) where
  BoundingBox3d bx by bz - Point3d px py pz = VectorBox3d (bx - px) (by - py) (bz - pz)

constant :: Point3d units coordinates -> BoundingBox3d units coordinates
...
hull4 :: Point3d units coordinates -> Point3d units coordinates -> Point3d units coordinates -> Point3d units coordinates -> BoundingBox3d units coordinates
