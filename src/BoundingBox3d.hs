...
instance Bounds (BoundingBox3d (Coordinates space units)) where
...
instance (units ~ units', space ~ space') => Subtraction (Point3d (Coordinates space units)) (BoundingBox3d (Coordinates space' units')) (VectorBox3d (Coordinates space units)) where
  Point3d px py pz - BoundingBox3d bx by bz = VectorBox3d (px - bx) (py - by) (pz - bz)

instance (units ~ units', space ~ space') => Subtraction (BoundingBox3d (Coordinates space units)) (Point3d (Coordinates space' units')) (VectorBox3d (Coordinates space units)) where
  BoundingBox3d bx by bz - Point3d px py pz = VectorBox3d (bx - px) (by - py) (bz - pz)

constant :: Point3d (Coordinates space units) -> BoundingBox3d (Coordinates space units)
...
hull2
  ...
  -> Point3d (Coordinates space units)
  -> BoundingBox3d (Coordinates space units)
...
interpolate :: BoundingBox3d (Coordinates space units) -> Float -> Float -> Float -> Point3d (Coordinates space units)
