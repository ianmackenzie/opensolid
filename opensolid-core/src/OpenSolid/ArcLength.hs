...
parameterization dsdt d2sdt2 = do
  ...
  let (isConstant, hasDegenerateStart, hasDegenerateEnd) =
...
buildTree
  ...
  coarseEstimate = do
    ...
    let includesDegenerateStart = hasDegenerateStart && tStart == 0.0
    let includesDegenerateEnd = hasDegenerateEnd && tEnd == 1.0
    if (depth >= 10 || converged) && not (includesDegenerateStart && includesDegenerateEnd)
      then do
        let d2sdt2Start = d2sdt2 tStart
        let d2sdt2End = d2sdt2 tEnd
        let segmentLength = fineEstimate
        if
          | includesDegenerateStart -> do
