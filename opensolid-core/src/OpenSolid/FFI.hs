...
import OpenSolid.Prelude hiding (Type, pattern NonEmpty, pattern Number, pattern Sign)
...
store ptr offset value = do
  ...
  case representation proxy of
    ...
    ListRep @item -> do
      ...
      IO.forEachWithIndex value \index item -> store itemsPtr (index * itemSize) item
