...
def tuple2_reader(
    read_a: Callable[[c_void_p], A], read_b: Callable[[c_void_p], B]
) -> Callable[[c_void_p], Tuple[A, B]]:
...
def result_reader(
    read_error: Callable[[c_int8, c_void_p], Exception],
    read_success: Callable[[c_void_p], A],
) -> Callable[[c_void_p], A]:
...
class Bounds2d:
    ...
    def intersection(self, bounds1: Bounds2d) -> Optional[Bounds2d]:
        return maybe_reader(Bounds2d)(
            lib.opensolid_bounds2d_intersection(bounds1.ptr, self.ptr)
        )
...
class Direction2d:
    ...
    def __repr__(self) -> str:
        return (
            "Direction2d("
            + str(self.x_component())
            + ", "
            + str(self.y_component())
            + ")"
        )
...
class Range:
    ...
    def endpoints(self) -> Tuple[float, float]:
        return tuple2_reader(read_float, read_float)(
            lib.opensolid_range_endpoints(self.ptr)
        )
    ...
    def intersection(self, range1: Range) -> Optional[Range]:
        return maybe_reader(Range)(
            lib.opensolid_range_intersection(range1.ptr, self.ptr)
        )
...
class Vector2d:
    ...
    def __repr__(self) -> str:
        return (
            "Vector2d(" + str(self.x_component()) + ", " + str(self.y_component()) + ")"
        )
    ...
    def direction(self, tolerance: Optional[float] = None) -> Direction2d:
        return result_reader(Vector2d.IsZero.from_tag, Direction2d)(
            lib.opensolid_vector2d_direction(read_tolerance(tolerance), self.ptr)
        )
    ...
    ) -> Tuple[float, Direction2d]:
        return result_reader(
            Vector2d.IsZero.from_tag, tuple2_reader(read_float, Direction2d)
        )(
            lib.opensolid_vector2d_magnitude_and_direction(
                read_tolerance(tolerance), self.ptr
            )
        )
