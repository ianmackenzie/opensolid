...
data Cache segment = Cache U.Bounds ~segment

data Tree segment
  = Tree (Cache segment) (Cache segment) ~(Tree segment) ~(Tree segment)

tree :: (U.Bounds -> segment) -> Tree segment
tree compute = buildTree compute U.domain

expand :: U.Bounds -> U.Bounds
expand (Range low high) =
  let expansion = 0.5 * (high - low)
   in Range.unsafe (Float.max 0.0 (low - expansion)) (Float.min 1.0 (high + expansion))

buildTree :: (U.Bounds -> segment) -> U.Bounds -> Tree segment
...
solve ::
  (U.Bounds -> segment -> Bool) ->
  (U.Bounds -> segment -> Fuzzy resolution) ->
  (U.Bounds -> segment -> resolution -> Maybe solution) ->
  Tree segment ->
  (List solution, List U.Bounds) ->
  (List solution, List U.Bounds)
...
solve2 ::
  (Show solution) =>
  (U.Bounds -> U.Bounds -> segment -> segment -> Bool) ->
  (U.Bounds -> U.Bounds -> segment -> segment -> Fuzzy resolution) ->
  (U.Bounds -> U.Bounds -> segment -> segment -> resolution -> Maybe solution) ->
  Tree segment ->
  Tree segment ->
  (List solution, List (U.Bounds, U.Bounds)) ->
  (List solution, List (U.Bounds, U.Bounds))
