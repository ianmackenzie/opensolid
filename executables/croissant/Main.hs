...
main = Tolerance.using Length.nanometer do
  ...
  let minorRadius =
        Curve1D.hermite Length.zero [k] Length.zero [negative k] `compose` SurfaceFunction1D.u
  let theta = Curve1D.interpolateFrom (Angle.degrees 45) (Angle.degrees 315) `compose` SurfaceFunction1D.u
