...
testIntervalArithmetic = do
  let intervalDifference =
        Interval (Length.meters 2.0) (Length.meters 3.0) - Length.centimeters 50.0
  log "Interval difference" intervalDifference
  let intervalProduct =
        Length.centimeters 20.0 * Interval (Length.meters 2.0) (Length.meters 3.0)
...
testTransformation = do
  let rotatedAxis = Axis2D.rotateAround (Point2D.meters 1.0 0.0) Angle.quarterTurn Axis2D.x
...
testListOperations = do
  ...
  let intervals = List.successive Interval [1.0, 2.0, 3.0, 4.0]
...
drawBounds :: Bounds2D Meters -> Svg
